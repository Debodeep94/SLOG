...
'''
chex = pd.read_csv('/nfs/data_chaos/dbanerjee/my_data/R2Gen/impression_chex.csv')
chex = chex.drop([ 'Reports'], axis=1) #'No Finding','Support Devices',
chexpert_cols=list(chex.columns[1:])

path = '/nfs/data_chaos/dbanerjee/my_data/R2Gen/surrogate/data/lps/'
tensors_gt=torch.load(path+'tensors_gt_emb.pt',map_location="cpu")
tensors_preds=torch.load(path+'tensors_preds_emb.pt',map_location="cpu")
seq_lens_gt=torch.load(path+'seq_lens_gt.pt',map_location="cpu")
seq_lens_preds=torch.load(path+'seq_lens_pred.pt',map_location="cpu")
#weight_values=torch.load(path+'weight_values.pt')
y_vals=torch.load(path+'surrogate_gt_labels.pt',map_location="cpu")
print('tensor length: ', len(tensors_gt))
# train test split

#weights, input_size, output_size, train_main_dataset, test_dataset = first_surr_split(tensors, seq_lens, weight_values,y_vals, 0.9)
# modelling first surrogate
X_gt, X_pred, Y, seq_X_gt, seq_X_pred = prepare_data(tensors_gt, tensors_preds, seq_lens_gt,seq_lens_preds, y_vals)
print(X_gt.size())
...
for train_idx, val_idx in tqdm(kf.split(X_gt)):
    ...
    x_train, x_val_gt, x_val_pred = X_gt[train_idx], X_gt[val_idx], X_pred[val_idx]
    y_train, y_val = Y[train_idx], Y[val_idx]
    seq_len_train, seq_len_val_gt, seq_len_val_preds = seq_X_gt[train_idx], seq_X_gt[val_idx], seq_X_pred[val_idx]
    weights = count_weights(y_train)
    # PyTorch DataLoader
    train_dataset = TensorDataset(x_train, y_train, seq_len_train)
    train_loader = DataLoader(train_dataset, batch_size=512, shuffle=True)

    val_dataset = TensorDataset(x_val_gt, x_val_pred, y_val, seq_len_val_gt, seq_len_val_preds)
    ...
    for epoch in range(num_epochs):
        ...
        start = time.time()
        with torch.no_grad():
            for (batch_inputs_gt,batch_inputs_preds, batch_targets, batch_seq_lens_gt, batch_seq_lens_pred) in val_loader:
                ...
                batch_inputs_gt,batch_inputs_preds, batch_targets, batch_seq_lens = batch_inputs_gt.to(device),batch_inputs_preds.to(device), batch_targets.to(device), batch_seq_lens.to(device)
                outputs_gt = model(batch_inputs_gt, batch_seq_lens_gt)
                outputs_pred = model(batch_inputs_preds, batch_seq_lens_pred)
        ...
        if current_patience >= patience:
            ...
            torch.save(val_accs_gt, path+'val_accs_gt_emb.pt')
            torch.save(val_accs_pred, path+'val_accs_pred_emb.pt')
            torch.save(val_ind_out_gt, path+'val_ind_out_gt.pt')
            torch.save(val_ind_out_preds, path+'val_ind_out_preds.pt')
            torch.save(all_true, path+'all_true_sur1_gt.pt')
            #val_inputs.clear()
            #all_acc.clear()
            #val_batch_seq.clear()
...
torch.save(sur2_data_gt, path+'sur2_data_gt_emb.pt')#.append(x_val_gt)
torch.save(sur2_data_pred, path+'sur2_data_pred_emb.pt')
torch.save(val_seq_gt, path+'val_seq_gt_emb.pt')
torch.save(val_seq_pred, path+'val_seq_pred_emb.pt')
...
# save the vectors
torch.save(fold_train_losses_padded, 'fold_train_losses_padded_emb.pt')
torch.save(fold_val_losses_padded_gt, 'fold_val_losses_padded_gt_emb.pt')
torch.save(fold_val_losses_padded_pred, 'fold_val_losses_padded_pred_emb.pt')

torch.save(all_val_f1_pred, 'all_val_f1_pred.pt')
torch.save(all_val_f1_gt, 'all_val_f1_gt.pt')
...
#torch.save(fold_train_losses_padded, 'fold_train_losses_padded.pt')
#torch.save(fold_val_losses_padded, 'fold_val_losses_padded.pt')
#print(all_folds)