...
class LSTM_Attn(nn.Module):
    ...
    def forward(self, encoded_captions, caption_lengths): #here x is encoded captions
        ...
        features = [attn(output, padding_mask) for attn in self.attns]
...
class AttentionVisualSurrogate(nn.Module):
    def __init__(self, image_embedding_dim,text_embedding_dim,
        ...
        self.fc1 = nn.Linear(image_embedding_dim + 2*text_embedding_dim, hidden_dim)
    ...
    def forward(self, images, encoded_captions, caption_lengths):
        ...
        # image_features= self.relu(image_features)
        # image_features=self.relu(image_features)
        text_features = self.text_extractor(encoded_captions,caption_lengths )
        text_features = text_features.mean(dim=1)  # Average over the sequence length
