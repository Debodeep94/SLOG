...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, init_state,init_latent,  init_logprobs, *args, **kwargs):

        # function computes the similarity score to be augmented
        
        def add_diversity(beam_seq_table, logprobs, t, divm, diversity_lambda, bdash):
            #print('beam_seq_table: ', beam_seq_table)
            local_time = t - divm
        ...
        def beam_step(logprobs,latent,  unaug_logprobs, beam_size, t, beam_seq,beam_latent, beam_seq_logprobs, beam_logprobs_sum, state):
            # INPUTS:
            ...
            latent_cloned=latent
            batch_size = beam_logprobs_sum.shape[0]
            vocab_size = logprobs.shape[-1]
            #print('logprobs1: ', logprobs.size())
            logprobs = logprobs.reshape(batch_size, -1, vocab_size)  # NxbxV #this is converting the 24 to batch*bdash*vocab
            #print('logprobs2: ', logprobs)
            #print('logprobs2: ', logprobs.size()) 
            # Do the same for the latents
            latent = latent.reshape(batch_size, -1, 512) # You have to automate the 512
            if t == 0:
            ...
            if t > 0:
                ...
                beam_latent = beam_latent.gather(1, beam_ix.unsqueeze(-1).unsqueeze(-1).expand_as(
                    beam_latent))
            ...
            beam_logprobs = unaug_logprobs.reshape(batch_size, -1, vocab_size).gather(1,
                                                                                      beam_ix.unsqueeze(-1).expand(-1,
                                                                                                                   -1,
                                                                                                                  vocab_size))  # NxbxV
            beam_temp_latent = latent_cloned.reshape(batch_size, -1, 512).gather(1,beam_ix.unsqueeze(-1).expand(-1,-1,512))  # Nxbxh, h:hidden_dim
            assert (_tmp_beam_logprobs == beam_logprobs).all()
            beam_seq_logprobs = torch.cat([
                beam_seq_logprobs,
                beam_logprobs.reshape(batch_size, -1, 1, vocab_size)], 2)
            beam_latent = torch.cat([
                beam_latent,
                beam_temp_latent.reshape(batch_size, -1, 1, 512)], 2)
            new_state = [None for _ in state]
            for _ix in range(len(new_state)):
                #  copy over state in previous beam q to new beam at vix
                new_state[_ix] = state[_ix][:, state_ix]
            state = new_state
            return beam_seq, beam_seq_logprobs, beam_logprobs_sum,beam_latent, state
        ...
        beam_seq_logprobs_table = [torch.FloatTensor(batch_size, bdash, 0, self.vocab_size + 1).to(device) for _ in
                                   range(group_size)]
        beam_latent_table = [torch.FloatTensor(batch_size, bdash, 0, 512).to(device) for _ in
                                   range(group_size)]
        ...
        latent_table = [init_latent.clone() for _ in range(group_size)]
        # END INIT
        ...
        for t in range(self.max_seq_length + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.max_seq_length + divm - 1:
                    ...
                    latent = latent_table[divm]
                    #print('logprobs: ', logprobs)
                    #print('logprobs size: ', logprobs.size())
                    ...
                    latent_table[divm], logprobs_table[divm],  state_table[divm] = self.get_logprobs_state(it.cuda(), *(
                            args[divm] + [state_table[divm]]))
                    #print('latent on caption size: ', latent_table[divm].size())
                    logprobs_table[divm] = F.log_softmax(logprobs_table[divm] / temperature, dim=-1)
                      
        # all beams are sorted by their log-probabilities
        #with torch.no_grad(): # added by me
        ...
        for t in range(self.max_seq_length + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.max_seq_length + divm - 1:
                    ...
                    _, logprobs_table[divm], state_table[divm] = self.get_logprobs_state(it.cuda(), *(
                            args[divm] + [state_table[divm]]))
                    logprobs_table[divm] = F.log_softmax(logprobs_table[divm] / temperature, dim=-1)
