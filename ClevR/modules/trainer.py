...
class BaseTrainer(object):
    ...
    def train(self):
        ...
        plt.savefig('/home/debodeep.banerjee/R2Gen/plots/train_imp_n_find.png')
        return log
        self._print_best()
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        with torch.no_grad():
            val_gts, val_res = [], []
            for batch_idx, (images_id, images, reports_ids, reports_masks, seq_length) in tqdm(enumerate(self.val_dataloader)):
                images, reports_ids, reports_masks = images.to(self.device), reports_ids.to(self.device), reports_masks.to(self.device)
            ...
            log.update(**{'val_' +'PRECISION_MICRO': val_prec_micro})
            log.update(**{'val_' +'PRECISION_MACRO': val_prec_macro})
            log.update(**{'val_' +'RECALL_MICRO': val_rec_micro})
            log.update(**{'val_' +'RECALL_MACRO': val_rec_macro})
            log.update(**{'val_' + k: v for k, v in val_met.items()})
