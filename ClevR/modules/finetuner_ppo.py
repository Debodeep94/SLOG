...
class BaseTrainer(object):
    def __init__(self, model, criterion,ppo_criterion, metric_ftns, optimizer,ppo_optimizer, args):
        self.args = args
...
class FineTuner(BaseTrainer):
    def __init__(self, model, criterion,ppo_criterion, metric_ftns, optimizer,ppo_optimizer, args, lr_scheduler, train_dataloader, val_dataloader):
        super(FineTuner, self).__init__(model, criterion,ppo_criterion, metric_ftns,
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (images_id, images, reports_ids, reports_masks, seq_length) in tqdm(enumerate(self.train_dataloader)):
            ...
            _, _, gs_logps_ref = self.model_ref(images, reports_ids, mode='gumbel')
            ground_truths = self.model.tokenizer.decode_batch(reports_ids[:, 1:].cpu().numpy())
            #lat_vec_ft, seq_ft, gs_logps_ft = self.model(images_ft, mode='sample')
            #print('gs_lps:', gs_logps_tr)
            ...
            surrogate = LSTM_Attn(embedded_sentence_tr.size(-1),256, 2)#,num_layers, output_size,3,dropout_rate=0.2)
            ...
            print('surr train score: ', predicted_tr.size())
            tot_surr_loss+=surr_tr_score.item()
            print('surrogate loss: ', surr_tr_score)
            surrogate_loss = self.surr_weight*surr_tr_score
            # ppo_loss = self.ce_weight*ppo(gs_logps_tr,gs_logps_ref)
            # hdm_loss =  ppo_loss-surrogate_loss #rlhf loss
            ...
            ground_truths = self.model.tokenizer.decode_batch(reports_ids[:, 1:].cpu().numpy())
            val_met, val_met_ind = self.metric_ftns({i: [gt] for i, gt in enumerate(ground_truths)},
                                    {i: [re] for i, re in enumerate(reports)})
            print(val_met_ind)
            reward = torch.tensor(val_met_ind['BLEU_4'])
            values = torch.ones_like(reward)
            advantages=compute_advantages(reward, values)
            print(reward.size())
            ...
            for _ in tqdm(range(1)):
                # Forward pass
                # Compute PPO loss
                loss_ppo = ppo_loss(gs_logps_ref, gs_logps_tr, advantages)
                print(f'loss ppo: {loss_ppo}')
                # Backward pass for PPO
                self.ppo_optimizer.zero_grad()
                loss_ppo.backward(retain_graph=True)
                self.ppo_optimizer.step()
            
            probs_p = torch.exp(gs_logps_tr)
            probs_q = torch.exp(gs_logps_ref)

            # Compute KL divergence
            kl_div = torch.mean(probs_p * (gs_logps_tr - gs_logps_ref))

            # hybrid+=hdm_loss.item()
            hdm_loss = kl_div-surrogate_loss
        ...
        with torch.no_grad():
            ...
            for batch_idx_val, (images_id, images, reports_ids, reports_masks, seq_length) in enumerate(self.val_dataloader):
                ...
                surrogate_checkpoint = torch.load(self.surrogate_model)   
                surrogate = LSTM_Attn(embedded_sentence_val.size(-1), 256, 2)#,num_layers, output_size,3,dropout_rate=0.2)
                surrogate = surrogate.to(self.device)
                surrogate.load_state_dict(surrogate_checkpoint)
