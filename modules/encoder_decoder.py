...
class Transformer(nn.Module):
    ...
    def decode(self, hidden_states, src_mask, tgt, tgt_mask):
        ...
        #print('embed', self.tgt_embed(tgt))
        #print('embed size', self.tgt_embed(tgt).size() )
...
class EncoderDecoder(AttModel):
    ...
    def forward_(self, fc_feats, att_feats, seq=None, att_masks=None):
        #print('entering forward_ function')
        if seq is not None:
            out, outputs = self._forward(fc_feats, att_feats, seq, att_masks)
        else:
            out, outputs = self._forward(fc_feats, att_feats, att_masks)
        return outputs

    def _forward(self, fc_feats, att_feats, seq, att_masks=None):

        if seq is not None:
            #print('Sequence in not None')
            att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks, seq)
            out = self.model(att_feats, seq, att_masks, seq_mask)
        else:
           # print('Sequence in None')
