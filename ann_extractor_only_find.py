...
class MimicAnnotation():
    ...
    def report(self, report_path):
        ...
        #print(i)
            with open(i) as f:
                ...
                if findings_start != -1: #or impression_start != -1:
                    subject = os.path.splitext(os.path.basename(i))[0]
                    #rep_sub_id.append(subject) this is for find or imp
                    rep_sub_id.append(subject)
                    text = report[findings_start+9:]  # extract text after 'IMPRESSION:' (assuming 11 is the length of 'IMPRESSION:')
                    #print(text)
                    if text == ' ':
                        print('blank report fiund, printing path..')
                        print(i)
                    else:
                        end_pos = 0
                        count = 0
                        ...
                        final_reports.append(text) 

        print('length of collected report:', len(rep_sub_id))
        print('length of collected report(obtained from list final reports):', len(final_reports))
        dicts_for_reports=[]
        for i in range(len(rep_sub_id)):
            #print(len(final_reports))
            report_dict={'id':rep_sub_id[i], 'report': final_reports[i] }
            #print(report_dict)
    ...
    def images (self, image_path, report_path):
        ...
        for report in tqdm(dicts_for_reports):
            for image in dicts_for_images:
                if report['id'] == image['id']:
                    ...
                    image_report_dict['report'] = report['report']
        ...
        merged_dict = [d for d in merged_dict if d.get('report') != ' ']
        ...
        out_file = open(r"/home/debodeep.banerjee/R2Gen/data/mimic/only_find_new.json", "w")
